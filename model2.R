...
FREQ_FORMULA <- claim_n ~ risk_tier + off_campus + sprinklered + greek + dist_band
SEV_FORMULA  <- avg_paid_cap ~ risk_tier + off_campus + sprinklered + greek + dist_band
...
pricing_base <- pricing_table %>% select(coverage, risk_tier, prem_renew, prem_new)
...
write.csv(student_totals, "student_totals_model2.csv", row.names = FALSE)
